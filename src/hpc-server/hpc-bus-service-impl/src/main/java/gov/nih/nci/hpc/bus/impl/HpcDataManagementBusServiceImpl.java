...
public class HpcDataManagementBusServiceImpl implements HpcDataManagementBusService {
	...
	public HpcArchiveResponseDTO archiveDataObject(String path) throws HpcException {
		...
		dataTransferService.archiveDataObject(metadata.getArchiveLocation(), HpcDataTransferType.S_3, metadata.getConfigurationId());

		// TODO YURI Record the data object requested in audit table.
		
		// Record the archive request in DB.
		HpcNciAccount invokerNciAccount = securityService.getRequestInvoker().getNciAccount();
		String archiveTaskId = dataManagementService.archiveDataObjects(invokerNciAccount.getUserId(),
				"", paths);
		

		// Create and return a DTO with the request receipt.
		HpcArchiveResponseDTO responseDTO = new HpcArchiveResponseDTO();
		responseDTO.setTaskId(archiveTaskId);
	...
	public HpcArchiveResponseDTO archiveCollection(String path) throws HpcException {
		...
		// Submit a archive transfer request.
		dataTransferService.archiveCollection(path, HpcDataTransferType.S_3, metadata.getConfigurationId());
		// TODO YURI Record the collection requested in audit table.
		
		List<String> paths = getDataObjectsUnderPath(collection);
		
		// Record the archive request in DB and obtain a task Id
		HpcNciAccount invokerNciAccount = securityService.getRequestInvoker().getNciAccount();
		String archiveTaskId = dataManagementService.archiveDataObjects(invokerNciAccount.getUserId(),
				"", paths);
	...
	@Override
	public HpcArchiveResponseDTO archiveDataObjectsOrCollections(HpcBulkDataObjectArchiveRequestDTO archiveRequest)
			throws HpcException {
		// Input validation.
		if (archiveRequest == null) {
			throw new HpcException("Null archive request", HpcErrorType.INVALID_REQUEST_INPUT);
		}

		if (archiveRequest.getDataObjectPaths().isEmpty() && archiveRequest.getCollectionPaths().isEmpty()) {
			throw new HpcException("No data object or collection paths", HpcErrorType.INVALID_REQUEST_INPUT);
		}
		if (!archiveRequest.getDataObjectPaths().isEmpty() && !archiveRequest.getCollectionPaths().isEmpty()) {
			throw new HpcException("Both data object and collection paths provided",
		...
		String archiveTaskId = null;
		List<String> paths = new ArrayList<>();
		HpcBulkArchiveRequest bulkArchiveRequest = new HpcBulkArchiveRequest();
		if (!archiveRequest.getDataObjectPaths().isEmpty()) {
			// Submit a request to archive a list of data objects.

			// Validate all data object paths requested exist.
			for (String path : archiveRequest.getDataObjectPaths()) {
				if (dataManagementService.getDataObject(path) == null) {
					throw new HpcException("Data object doesn't exist: " + path, HpcErrorType.INVALID_REQUEST_INPUT);
				}
				if (StringUtils.isEmpty(path)) {
				...
				if (paths.stream().anyMatch(s -> path.equals(s))) {
					throw new HpcException("Duplicated path in archive requests list: " + path,
							HpcErrorType.INVALID_REQUEST_INPUT);
				}
				...
				paths.add(path);
				HpcBulkArchiveItem item = new HpcBulkArchiveItem();
				item.setPath(metadata.getArchiveLocation().getFileId());
				item.setConfigurationId(metadata.getConfigurationId());
				bulkArchiveRequest.getItems().add(item);
			}
						
			// Submit a data objects archive task.
			dataTransferService.archiveDataObjects(bulkArchiveRequest, HpcDataTransferType.S_3);
			// TODO YURI Record all data object paths requested in audit table.
			
			// Record archive request in DB
			archiveTaskId = dataManagementService.archiveDataObjects(invokerNciAccount.getUserId(),
					"", paths);
		} else {
			...
			// Validate all data object paths requested exist.
			boolean dataObjectExist = false;
			for (String path : archiveRequest.getCollectionPaths()) {
				HpcCollection collection = dataManagementService.getCollection(path, true);
				if (collection == null) {
					throw new HpcException("Collection doesn't exist: " + path, HpcErrorType.INVALID_REQUEST_INPUT);
				}
				...
				//TODO YURI for each data object, we need to check if archive request can be created (Cloudian only)
				HpcBulkArchiveItem item = new HpcBulkArchiveItem();
				String configurationId = metadataService
			...
			// Submit a data objects archive task.
			dataTransferService.archiveCollections(bulkArchiveRequest, HpcDataTransferType.S_3);
			// TODO YURI Record all collections requested in audit table.
	...
	private HpcSystemGeneratedMetadata validateDataObjectDownloadRequest(String path, boolean s3DownloadDestination,
		...
		if (!dataTransferStatus.equals(HpcDataTransferUploadStatus.ARCHIVED) && !dataTransferStatus.equals(HpcDataTransferUploadStatus.DEEP_ARCHIVE)) {
