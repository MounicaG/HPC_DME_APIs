...
public interface HpcDataManagementService {
   ...
   * implemented. iRODS has this capability, and there is a plan to use it. This is a temporary
   * solution to have a record of user attempts to delete or update data objects and collections.
   * When the permanent solution is implemented (using iRODS capability) this API method and the DAO
   * behind it should be retired.
   *
   ...
   * @param userId (Optional) userId if not executed in a user thread.
   */
  ...
  /**
   * Acquire permission of given user on a given collection. Ensures that even if given user has no
  ...
  /**
   * Set a co-ownership on an entity. Both the user-id and the system account will be assigned as
   * owners.
   *
  ...
  /**
   * Complete a bulk data object registration task: 1. Update task info in DB with results info.
   *
  ...
  /**
   * Find data management configuration ID for a given path. This is searched by matching the given
   * path to all configuration base paths. If any configuration base path is found to be the 'base
   * path' of the given path, then the corresponding configuration ID is returned
   *
   ...
   * @param result the registration request result (success/fail indicator).
   * @param message (Optional) If the registration failed, a message describing the failure.
   * @throws HpcException on service failure.
   */
