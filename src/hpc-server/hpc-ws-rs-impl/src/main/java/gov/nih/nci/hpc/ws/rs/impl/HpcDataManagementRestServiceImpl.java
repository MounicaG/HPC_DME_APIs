 ...
 */
public class HpcDataManagementRestServiceImpl extends HpcRestServiceImpl implements HpcDataManagementRestService {
	// ---------------------------------------------------------------------//
	// Constants
	// ---------------------------------------------------------------------//

	// The attribute name to save download file path on the message context.
	public static final String DATA_OBJECT_DOWNLOAD_FILE_MC_ATTRIBUTE = "gov.nih.nci.hpc.ws.rs.impl.HpcDataManagementRestService.dataObjectDownloadFile";

	// The attribute name to save download file path on the message context.
	public static final String DATA_OBJECT_DOWNLOAD_TASK_ID_MC_ATTRIBUTE = "gov.nih.nci.hpc.ws.rs.impl.HpcDataManagementRestService.dataObjectDownloadTaskId";
	...
	public Response interrogatePathRef(String path) {
		try {
			...
			try {
				...
				final String errMsg = String.format("Failure during conversion of Map to JSON: %s",
	...
	@Override
	public Response registerCollection(String path, HpcCollectionRegistrationDTO collectionRegistration) {
	...
	public Response getCollection(String path, Boolean list, Boolean includeAcl) {
		...
		try {
			HpcCollectionDTO collection = dataManagementBusService.getCollection(toNormalizedPath(path), list,
	...
	public Response getCollectionChildren(String path) {
		HpcCollectionListDTO collections = new HpcCollectionListDTO();
		try {
			HpcCollectionDTO collection = dataManagementBusService.getCollectionChildren(toNormalizedPath(path));
			if (collection != null) {
				collections.getCollections().add(collection);
			}
	...
	public Response downloadCollection(String path, HpcDownloadRequestDTO downloadRequest) {
		// This API is deprecated and replaced by a new download API (which supports
		// additional S3
	...
	public Response downloadCollection(String path,
		...
		try {
			downloadResponse = dataManagementBusService.downloadCollection(toNormalizedPath(path), downloadRequest);
	...
	public Response setCollectionPermissions(String path, HpcEntityPermissionsDTO collectionPermissionsRequest) {
		...
		try {
			permissionsResponse = dataManagementBusService.setCollectionPermissions(toNormalizedPath(path),
	...
	public Response getCollectionPermission(String path, String userId) {
		...
		try {
			hpcUserPermissionDTO = dataManagementBusService.getCollectionPermission(toNormalizedPath(path), userId);
	...
	@Deprecated
	@Override
	public Response registerDataObject(String path, HpcDataObjectRegistrationRequestDTO dataObjectRegistration,
	...
	public Response registerDataObject(String path,
		...
		try {
			...
			responseDTO = dataManagementBusService.registerDataObject(toNormalizedPath(path), dataObjectRegistration,
					dataObjectFile);
		...
		if (registered) {
			...
			return okResponse(
					responseDTO.getUploadRequestURL() != null || responseDTO.getMultipartUpload() != null ? responseDTO
							: null,
					false);
		}
	}
	...
	public Response getDataObjectsRegistrationStatus(String taskId) {
		gov.nih.nci.hpc.dto.datamanagement.v2.HpcBulkDataObjectRegistrationStatusDTO registrationStatus = null;
		try {
	...
	public Response downloadDataObject(String path, HpcDownloadRequestDTO downloadRequest,
		...
		// a dedicated API to generate download URL. This API should be removed in the
		// future.
		if (downloadRequest != null && downloadRequest.getGenerateDownloadRequestURL() != null
				&& downloadRequest.getGenerateDownloadRequestURL()) {
			if (downloadRequest.getDestination() != null || downloadRequest.getDestinationOverwrite() != null) {
	...
	public Response generateDownloadRequestURL(String path) {
		...
		try {
			downloadResponse = dataManagementBusService.generateDownloadRequestURL(toNormalizedPath(path));
	...
	public Response downloadDataObject(String path,
		...
		HpcDataObjectDownloadResponseDTO downloadResponse = null;
		try {
			downloadResponse = dataManagementBusService.downloadDataObject(toNormalizedPath(path), downloadRequest);
	...
	public Response setDataObjectPermissions(String path, HpcEntityPermissionsDTO dataObjectPermissionsRequest) {
		HpcEntityPermissionsResponseDTO permissionsResponse = null;
		try {
			permissionsResponse = dataManagementBusService.setDataObjectPermissions(toNormalizedPath(path),
	...
	public Response getDataObjectPermission(String path, String userId) {
		HpcUserPermissionDTO hpcUserPermissionDTO = null;
		try {
			hpcUserPermissionDTO = dataManagementBusService.getDataObjectPermission(toNormalizedPath(path), userId);
	...
	@Override
	public Response archiveDataObject(String path) {
		HpcArchiveResponseDTO archiveResponse = null;
		try {
			archiveResponse = dataManagementBusService.archiveDataObject(path);
			
		} catch (HpcException e) {
			return errorResponse(e);
		}

		return okResponse(archiveResponse, true);
	}

	@Override
	public Response archiveCollection(String path) {
		HpcArchiveResponseDTO archiveResponse = null;
		try {
			archiveResponse = dataManagementBusService.archiveCollection(path);
	...
	@Override
	public Response archiveDataObjectsOrCollections(HpcBulkDataObjectArchiveRequestDTO archiveRequest) {
		HpcArchiveResponseDTO archiveResponse = null;
		try {
			archiveResponse = dataManagementBusService.archiveDataObjectsOrCollections(archiveRequest);
	...
	private File toFile(InputStream dataObjectInputStream) throws HpcException {
		...
		File dataObjectFile = FileUtils.getFile(multipartProvider.getTempDirectory(), UUID.randomUUID().toString());
		try {
	...
	/**
	 * Create a Response object out of the DTO. Also set the download file path on
	 * the message context, so that the cleanup interceptor can remove it after
	 * requested file reached the caller.
	 *
	...
	private Response downloadResponse(HpcDataObjectDownloadResponseDTO downloadResponse,
		...
		if (downloadResponse.getDestinationFile() != null) {
			...
			messageContext.put(DATA_OBJECT_DOWNLOAD_FILE_MC_ATTRIBUTE, downloadResponse.getDestinationFile());
			messageContext.put(DATA_OBJECT_DOWNLOAD_TASK_ID_MC_ATTRIBUTE, downloadResponse.getTaskId());
			response = okResponse(downloadResponse.getDestinationFile(), MediaType.APPLICATION_OCTET_STREAM_TYPE);
		} else {
	 ...
	 */
	private gov.nih.nci.hpc.dto.datamanagement.v2.HpcDownloadRequestDTO toV2(HpcDownloadRequestDTO downloadRequest) {
		if (downloadRequest == null) {
			return null;
		}

		gov.nih.nci.hpc.dto.datamanagement.v2.HpcDownloadRequestDTO v2DownloadRequest = new gov.nih.nci.hpc.dto.datamanagement.v2.HpcDownloadRequestDTO();
	...
	private gov.nih.nci.hpc.dto.datamanagement.v2.HpcBulkDataObjectDownloadRequestDTO toV2(
		...
		gov.nih.nci.hpc.dto.datamanagement.v2.HpcBulkDataObjectDownloadRequestDTO v2DownloadRequest = new gov.nih.nci.hpc.dto.datamanagement.v2.HpcBulkDataObjectDownloadRequestDTO();
	...
	private gov.nih.nci.hpc.dto.datamanagement.v2.HpcDataObjectRegistrationRequestDTO toV2(
		...
		gov.nih.nci.hpc.dto.datamanagement.v2.HpcDataObjectRegistrationRequestDTO v2DataObjectRegistration = new gov.nih.nci.hpc.dto.datamanagement.v2.HpcDataObjectRegistrationRequestDTO();
		...
		v2DataObjectRegistration.setCreateParentCollections(dataObjectRegistration.getCreateParentCollections());
		...
		v2DataObjectRegistration.setGenerateUploadRequestURL(dataObjectRegistration.getGenerateUploadRequestURL());
		v2DataObjectRegistration.getMetadataEntries().addAll(dataObjectRegistration.getMetadataEntries());
	...
	private gov.nih.nci.hpc.dto.datamanagement.v2.HpcBulkDataObjectRegistrationRequestDTO toV2(
		...
		gov.nih.nci.hpc.dto.datamanagement.v2.HpcBulkDataObjectRegistrationRequestDTO v2BulkDataObjectRegistrationRequest = new gov.nih.nci.hpc.dto.datamanagement.v2.HpcBulkDataObjectRegistrationRequestDTO();
		...
		bulkDataObjectRegistrationRequest.getDataObjectRegistrationItems().forEach(dataObjectRegistrationItem -> {
			gov.nih.nci.hpc.dto.datamanagement.v2.HpcDataObjectRegistrationItemDTO v2DataObjectRegistrationItem = new gov.nih.nci.hpc.dto.datamanagement.v2.HpcDataObjectRegistrationItemDTO();
			v2DataObjectRegistrationItem.setCallerObjectId(dataObjectRegistrationItem.getCallerObjectId());
		...
		bulkDataObjectRegistrationRequest.getDirectoryScanRegistrationItems().forEach(directoryScanRegistrationItem -> {
			gov.nih.nci.hpc.dto.datamanagement.v2.HpcDirectoryScanRegistrationItemDTO v2DirectoryScanRegistrationItem = new gov.nih.nci.hpc.dto.datamanagement.v2.HpcDirectoryScanRegistrationItemDTO();
			...
			v2DirectoryScanRegistrationItem.setCallerObjectId(directoryScanRegistrationItem.getCallerObjectId());
			v2DirectoryScanRegistrationItem.setPathMap(directoryScanRegistrationItem.getPathMap());
			v2DirectoryScanRegistrationItem.setPatternType(directoryScanRegistrationItem.getPatternType());
	...
	private HpcBulkDataObjectRegistrationResponseDTO toV1(
		...
		HpcBulkDataObjectRegistrationResponseDTO bulkDataObjectRegistrationResponse = new HpcBulkDataObjectRegistrationResponseDTO();

		bulkDataObjectRegistrationResponse.setTaskId(v2BulkDataObjectRegistrationResponse.getTaskId());
		v2BulkDataObjectRegistrationResponse.getDataObjectRegistrationItems().forEach(v2DataObjectRegistrationItem -> {
			HpcDataObjectRegistrationItemDTO dataObjectRegistrationItem = new HpcDataObjectRegistrationItemDTO();
			dataObjectRegistrationItem.setCallerObjectId(v2DataObjectRegistrationItem.getCallerObjectId());
			dataObjectRegistrationItem
					.setCreateParentCollections(v2DataObjectRegistrationItem.getCreateParentCollections());
			...
			if (v2DataObjectRegistrationItem.getGlobusUploadSource() != null) {
				dataObjectRegistrationItem
						.setSource(v2DataObjectRegistrationItem.getGlobusUploadSource().getSourceLocation());
			}
			bulkDataObjectRegistrationResponse.getDataObjectRegistrationItems().add(dataObjectRegistrationItem);
		});
	...
	private HpcBulkDataObjectRegistrationStatusDTO toV1(
		...
		HpcBulkDataObjectRegistrationStatusDTO bulkDataObjectRegistrationStatus = new HpcBulkDataObjectRegistrationStatusDTO();

		bulkDataObjectRegistrationStatus.setInProgress(v2BulkDataObjectRegistrationStatus.getInProgress());
	...
	private HpcBulkDataObjectRegistrationTaskDTO toV1(
		...
		HpcBulkDataObjectRegistrationTaskDTO bulkDataObjectRegistrationTask = new HpcBulkDataObjectRegistrationTaskDTO();
		...
		bulkDataObjectRegistrationTask.getCompletedItems().addAll(v2BulkDataObjectRegistrationTask.getCompletedItems());
		bulkDataObjectRegistrationTask.getFailedItems().addAll(v2BulkDataObjectRegistrationTask.getFailedItems());
		...
		v2BulkDataObjectRegistrationTask.getFailedItemsRequest().forEach(v2DataObjectRegistrationItem -> {
			...
			dataObjectRegistrationItem.getDataObjectMetadataEntries()
					.addAll(v2DataObjectRegistrationItem.getDataObjectMetadataEntries());
			dataObjectRegistrationItem.setSource(v2DataObjectRegistrationItem.getGlobusUploadSource() != null
	...
	private HpcRegistrationSummaryDTO toV1(
		...
		v2RegistrationSummary.getActiveTasks().forEach(v2BulkDataObjectRegistrationTask -> registrationSummary
				.getActiveTasks().add(toV1(v2BulkDataObjectRegistrationTask)));
		v2RegistrationSummary.getCompletedTasks().forEach(v2BulkDataObjectRegistrationTask -> registrationSummary
				.getCompletedTasks().add(toV1(v2BulkDataObjectRegistrationTask)));
