...
public class HpcDataTransferProxyImpl implements HpcDataTransferProxy {
	...
	public HpcPathAttributes getPathAttributes(Object authenticatedToken, HpcFileLocation fileLocation, boolean getSize)
		...
		if (fileExists) {
			...
			try {
				...
				boolean directoryExists = s3Connection.getTransferManager(authenticatedToken).getAmazonS3Client()
	...
	@Override
	public void putLifecyclePolicy(Object authenticatedToken, HpcFileLocation archiveLocation,
			String prefix) throws HpcException {
        ...
        newRule.setFilter(new LifecycleFilter(new LifecyclePrefixPredicate(prefix)));

        try {
            AmazonS3 s3Client = s3Connection.getTransferManager(authenticatedToken).getAmazonS3Client();

            // Retrieve the configuration.
            BucketLifecycleConfiguration configuration = s3Client.getBucketLifecycleConfiguration(archiveLocation.getFileContainerId());
            ...
            s3Client.setBucketLifecycleConfiguration(archiveLocation.getFileContainerId(), configuration);

        } catch (AmazonServiceException e) {
        	throw new HpcException(
					"[S3] Failed to add a new rule to life cycle policy on bucket " + archiveLocation.getFileContainerId() + ":"
							+ prefix + e.getMessage(),
					HpcErrorType.DATA_TRANSFER_ERROR, s3Connection.getS3Provider(authenticatedToken), e);
        } catch (AmazonClientException e) {
			throw new HpcException(
	...
	private String downloadDataObject(Object authenticatedToken, HpcFileLocation archiveLocation,
		...
		if (!s3BucketAccessible) {
			throw new HpcException("Failed to access AWS S3 bucket: " + s3Destination.getDestinationLocation(),
